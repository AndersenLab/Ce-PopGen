...
for( k in unique(admix_pops$K)){
  ...
  for(i in 1:length(LETTERS[1:(k+1)])){
    if(i > k){
      branch_strains[[i]] <- analysis_k %>% dplyr::filter(frac_cluster < 0.9) %>% dplyr::pull(strain)
    } else {
      branch_strains[[i]] <- analysis_k %>% dplyr::filter(cluster == LETTERS[i], frac_cluster > 0.9) %>% dplyr::pull(strain)
    }
    
  }
  
  names(branch_strains) <- c(LETTERS[1:k],"admixed")
  
  tree_pt_h <- ggtree::groupOTU(btree, branch_strains)
  ...
  admix_kpops <- data.table::fread("Processed_Data/Admixture/GATK-STRELKA_Intersection_Complete_Ancestral_Pops.tsv") %>%
    dplyr::filter(K== k)
